...
def test_happy_path(
    ...
    with logged_in_user():
        response = client.put("/datasets/1", json=body, headers={"Authorization": "Fake token"})
    assert response.status_code == 200, response.json()
...
def test_post_editors(
    ...
    with logged_in_user():
        client.post("/persons", json={"name": "1"}, headers=headers)
        client.post("/persons", json={"name": "2"}, headers=headers)
        client.post("/persons", json={"name": "3"}, headers=headers)
...
def test_update_aiod_entry(
    ...
    response = client.put("/news/1", json=put_body, headers={"Authorization": "Fake token"})
    assert response.status_code == 200, response.json()
...
def test_relations_between_resources(
    ...
    response = client.put("/news/1", json=body, headers={"Authorization": "Fake token"})
    assert response.status_code == 200, response.json()
